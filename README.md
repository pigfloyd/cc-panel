...
| 状态 | 卡片背景 | 状态主色 | 动效 / 含义 |
|---|---|---|---|
| 工作中 | `#fff0e9` | 珊瑚橙 `#d97757` | 光带扫过、圆点呼吸；agent 正在执行任务 |
| 等待输入 | `#ffe9ed` | 红 `#dc1f45` | 边框和圆点脉冲；等待权限确认或用户回复 |
| 已完成 | `#e4f6ec` | 绿 `#10914f` | 回合结束，可以查看结果 |
| 出错 | `#fff2db` | 琥珀 `#c67a00` | 工具失败或停止失败 |
| 空闲 | `#f2f4f8` | 灰蓝 `#79828f` | 会话刚启动，尚未执行任务 |
| 已结束 | `#f1f3f6` | 灰 `#949ba5` | 会话正常退出，卡片半透明并在 15 秒后消失 |
| 进程已退出 | `#edeff3` | 深灰 `#7f8690` | agent 进程已不存在，卡片半透明 |
